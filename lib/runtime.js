import {CapabilityRouter} from './v2/core.js'
...
export class Router {
  ...
  async route(text, signal) {
    const capabilities=new CapabilityRouter(this.store.root)
    try {if(await capabilities.store.exists())return await capabilities.route(text,signal)} catch {return {kind:'fallback',reason:'capability-store-unavailable'}}

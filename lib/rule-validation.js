...
export async function validateCollection(store, target, { worker = new RuleWorker(), signal, expectedGeneration, coverage = {}, historyIds = [] } = {}) {
  const started = performance.now(), s = await store.read(), baseSuite = await loadRuleSuite(store), validation_id = randomUUID()
  const inherited=Object.values(s.active).flatMap(a=>Object.keys(s.proofs[a.validation_id]?.historyEvidence??{}))
  const replay=historyReplay(s,[...inherited,...historyIds])
  const suite=validateSuite({version:1,cases:[...baseSuite.cases,...replay.cases]})
  ...
  const proof = { validation_id, generation: s.generation, target: structuredClone(target), coverage, environment: environmentKey(s.config), suiteHash: hash(baseSuite), admissionHash: hash(suite), historyEvidence: replay.evidence,

import { executableChanged } from './rule-history.js'
...
export class RuleOperations {
  ...
  async validation(signal){
    ...
    if(c.composition){
      ...
      proof=await validateCollection(this.store,target,{...this.options,signal,expectedGeneration:s.generation,coverage,historyIds:c.source_ids})
    }else proof=await new RuleRegistry(this.store,{...this.options,historyIds:c.source_ids}).validate(c.latest_revision,signal)
  ...
  async call(action,data={},operationId=randomUUID(),signal){assert(operations.includes(action),'Unknown rule operation');return this.journal(action,data,operationId,async()=>{
    ...
    if(action==='create_rule'){
      // A seeded family continues from its stable implementation. Repeated
      // create calls select the existing draft instead of discarding progress.
      if(c.current_draft){const d=c.drafts[c.current_draft];return{draft_id:d.id,path:'/draft',base_revision:d.base_revision,reused:true}}
      return this.newDraft(c.rule_id,c.stable_revision??c.base_revision)
    }
    ...
    if(action==='activate_rule'){
      ...
      if(c.base_revision&&!c.composition)assert(executableChanged(s.packages[c.base_revision].files,s.packages[c.latest_revision].files),'No executable change: metadata-only republishing is not an update')

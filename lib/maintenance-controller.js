...
import { KERNEL_HASH, hash } from './rule-contract.js'
...
export class MaintenanceController {
  ...
  async next(){
    ...
    for(const[family,rows]of families){
      ...
      if(s.active[family]){
        const collection=hash(s.active),router=new ObjectRouter(this.store.root),novel=rows.filter(r=>!seen.has(r.id));let uncovered=false
        for(const row of novel){
          if(m.covered?.[row.id]===collection)continue
          const result=await router.route(row.request)
          if(result.kind!=='completed'||result.text!==row.output){uncovered=true;continue}
          await this.store.transaction(current=>{
            if(hash(current.active)!==collection)return
            const state=maintenanceState(current);state.covered??={};state.covered[row.id]=collection
            event(current,'maintenance-covered-history',{history_id:row.id,collection,routeMs:result.durationMs??null})
          })
        }
        if(!uncovered)continue
      }
      return createChain(this.store,{rule_id:family,source_ids:rows.slice(-100).map(r=>r.id),base_revision:s.active[family]?.revision??null,goal:`Extend the existing deterministic rule for currently uncovered verified requests in family ${family}. Replay fixtures, when supplied by the trusted evaluator, must pass alongside all existing regression cases. Preserve supported scope and existing correct behavior; abstain on unsupported constraints. Do not republish unchanged executable code.`})
    }
    return null
  }
  async run(chainId){

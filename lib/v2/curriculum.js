...
export class Curriculum {
 ...
 async next(){
  ...
  if(index===this.plan.tasks.length&&this.plan.finishControl==='agent'){await this.store.transaction(s=>{s.sessions[this.sid].curriculum.phase='awaiting_finish';event(s,'curriculum-tasks-complete',{sid:this.sid})});return {tasksComplete:true}}
 ...
 async control(a){return this.exclusive(async()=>{
  ...
  if(a.action==='finish_training'){assert(c.phase==='awaiting_finish'&&c.answers.length===this.plan.tasks.length,'Tasks must be completed and development closed');await this.store.transaction(s=>{s.sessions[this.sid].curriculum.phase='done';event(s,'curriculum-done',{sid:this.sid,trigger:'agent'})});return {done:true}}
  if(a.action==='begin_development'){
   assert(this.plan.mode==='autonomous'&&['task','awaiting_finish'].includes(c.phase),'Manual start only in autonomous task phase');assert(typeof a.reason==='string'&&a.reason.trim(),'Record why development is useful')
   await this.store.transaction(s=>{const x=s.sessions[this.sid].curriculum;x.resumePhase=c.phase;x.phase='development';x.decisions.push({index:c.index,action:a.action,reason:a.reason,call:session.calls.length,trigger:'agent'});event(s,'curriculum-development',{sid:this.sid,index:c.index,trigger:'agent',reason:a.reason})});return {phase:'development',resume:'Call end_development to resume this task.'}
  }
  if(a.action==='end_development'){
   assert(c.phase==='development','Not developing');await this.store.transaction(s=>{const x=s.sessions[this.sid].curriculum;x.decisions.push({index:c.index,action:a.action,reason:a.reason??'',call:session.calls.length});x.phase=x.resumePhase??'task'})
   return this.plan.mode==='batch'?this.next():c.resumePhase==='awaiting_finish'?{phase:'awaiting_finish',tasksComplete:true}:{phase:'task',taskId:this.plan.tasks[c.index].id}
  }
